...
struct MemoryProtectionUnit {
    ...
    struct Access final {
        ...
        constexpr Access()
            : whole{0U} {}
        /// Copy Constructor
        constexpr Access(Access const& other)
            : whole{other.whole} {}
        ...
        public:
            ...
            inline bool set_power2_size(std::uint32_t bytes) volatile {
                if (polyfill::is_power_of_two(bytes)) {
...
enum class ProtectedRegion : std::uint8_t {
    ...
    ProcessStack = 4U,      ///< Just the Process Stack area (comes after Main)
...
/// @param attribute
peripherals::MemoryProtectionUnit::Access make_access(peripherals::MemoryProtectionUnit::Attribute attribute);
