...
#ifndef STM32_F4XX_DEBUG__HPP_
#define STM32_F4XX_DEBUG__HPP_
...
struct Debug final {
    ...
    struct APB1Freeze final {
        /// Default Constructor
        APB1Freeze()
            : whole{0u} {}
        /// Copy Constructor from volatile
        APB1Freeze(APB1Freeze volatile const& other)
            : whole{other.whole} {}
        /// Copy Construction from nonvolatile
        APB1Freeze(APB1Freeze const& other)
            : whole{other.whole} {}
        /// Move Constructor is deleted
        APB1Freeze(APB1Freeze&&) = delete;
        /// Parameterized Constructor for constant references
        explicit APB1Freeze(uint32_t const& value)
            : whole{value} {}
        /// Parameterized Constructor for volatile references
        explicit APB1Freeze(uint32_t volatile& value)
            : whole{value} {}
        /// Destructor is empty
        ~APB1Freeze() = default;
        ...
        //+=MEMORY======================================+
        /// Move Assignment is deleted
        APB1Freeze& operator=(APB1Freeze&&) = delete;
        /// Assignment from a volatile to a non volatile copy of the register.
        /// @note Does not return a reference
        inline void operator=(APB1Freeze volatile& other) { whole = other.whole; }
        /// Assignment from a non volatile to a volatile register
        /// @note Does not return a reference
        inline void operator=(APB1Freeze const& other) volatile { whole = other.whole; }
        /// Copy Assign a complete value from the base type uint32_t into a volatile version
        /// This allows you to assign the whole register to a literal value or a known constant of the same type.
        /// @note Does not return a reference
        inline void operator=(uint32_t const& value) volatile { whole = value; }
    ...
    // Ensure the register is in standard layout format
    static_assert(std::is_standard_layout<APB1Freeze>::value, "Must be standard layout");
    // Ensure the sizeof the entire register is correct.
    static_assert(sizeof(APB1Freeze) == 4UL, "Must be this exact size");
    /// Debug MCU APB2 Freeze registe (DBGMCU_APB2_FZ)
    struct APB2Freeze final {
        /// Default Constructor
        APB2Freeze()
            : whole{0u} {}
        /// Copy Constructor from volatile
        APB2Freeze(APB2Freeze volatile const& other)
            : whole{other.whole} {}
        /// Copy Construction from nonvolatile
        APB2Freeze(APB2Freeze const& other)
            : whole{other.whole} {}
        /// Move Constructor is deleted
        APB2Freeze(APB2Freeze&&) = delete;
        /// Parameterized Constructor for constant references
        explicit APB2Freeze(uint32_t const& value)
            : whole{value} {}
        /// Parameterized Constructor for volatile references
        explicit APB2Freeze(uint32_t volatile& value)
            : whole{value} {}
        /// Destructor is empty
        ~APB2Freeze() = default;
        ...
        //+=MEMORY======================================+
        /// Move Assignment is deleted
        APB2Freeze& operator=(APB2Freeze&&) = delete;
        /// Assignment from a volatile to a non volatile copy of the register.
        /// @note Does not return a reference
        inline void operator=(APB2Freeze volatile& other) { whole = other.whole; }
        /// Assignment from a non volatile to a volatile register
        /// @note Does not return a reference
        inline void operator=(APB2Freeze const& other) volatile { whole = other.whole; }
        /// Copy Assign a complete value from the base type uint32_t into a volatile version
        /// This allows you to assign the whole register to a literal value or a known constant of the same type.
        /// @note Does not return a reference
        inline void operator=(uint32_t const& value) volatile { whole = value; }
    ...
    // Ensure the register is in standard layout format
    static_assert(std::is_standard_layout<APB2Freeze>::value, "Must be standard layout");
    // Ensure the sizeof the entire register is correct.
    static_assert(sizeof(APB2Freeze) == 4UL, "Must be this exact size");
    ...
    /// Debug MCU APB1 Freeze registe (DBGMCU_APB1_FZ)
    APB1Freeze apb1_freeze;    // offset 0x8UL
    /// Debug MCU APB2 Freeze registe (DBGMCU_APB2_FZ)
    APB2Freeze apb2_freeze;    // offset 0xcUL
...
static_assert(offsetof(Debug, apb1_freeze) == 0x8UL, "Must be located at this offset");
static_assert(offsetof(Debug, apb2_freeze) == 0xcUL, "Must be located at this offset");
