...
#ifndef STM32_F4XX_POWERCONTROLLER__HPP_
#define STM32_F4XX_POWERCONTROLLER__HPP_
...
struct PowerController final {
    ...
    struct Control final {
        ...
        struct Fields final {
            ...
            /// Power down deepsleep (PDDS)
            uint32_t power_down_deep_sleep           : 1;    // bit 1
            /// Clear wakeup flag (CWUF)
            uint32_t clear_wakeup_flag               : 1;    // bit 2
            /// Clear standby flag (CSBF)
            uint32_t clear_standby_flag              : 1;    // bit 3
            /// Power voltage detector enable (PVDE)
            uint32_t power_voltage_detector_enable   : 1;    // bit 4
            /// PVD level selection (PLS)
            ...
            /// Flash power down in Stop mode (FPDS)
            uint32_t flash_power_down_in_stop_mode   : 1;    // bit 9
            /// (reserved)
    ...
    /// power control/status register (CSR)
    struct ControlStatus final {
        /// Default Constructor
        ControlStatus()
            : whole{0u} {}
        /// Copy Constructor from volatile
        ControlStatus(ControlStatus volatile const& other)
            : whole{other.whole} {}
        /// Copy Construction from nonvolatile
        ControlStatus(ControlStatus const& other)
            : whole{other.whole} {}
        /// Move Constructor is deleted
        ControlStatus(ControlStatus&&) = delete;
        /// Parameterized Constructor for constant references
        explicit ControlStatus(uint32_t const& value)
            : whole{value} {}
        /// Parameterized Constructor for volatile references
        explicit ControlStatus(uint32_t volatile& value)
            : whole{value} {}
        /// Destructor is empty
        ...
        struct Fields final {
            /// Wakeup flag (WUF)
            uint32_t wakeup_flag                   : 1;    // bit 0
            /// Standby flag (SBF)
            uint32_t standby_flag                  : 1;    // bit 1
            /// PVD output (PVDO)
            uint32_t power_voltage_detector_output : 1;    // bit 2
            /// Backup regulator ready (BRR)
            uint32_t backup_regulator_ready        : 1;    // bit 3
            /// (reserved)
            uint32_t                               : 4;    // bits 4:7
            /// Enable WKUP pin (EWUP)
            uint32_t enable_wakeup_pin             : 1;    // bit 8
            /// Backup regulator enable (BRE)
            uint32_t backup_regulator_enable       : 1;    // bit 9
            /// (reserved)
            uint32_t                               : 4;    // bits 10:13
            /// Regulator voltage scaling output selection ready bit (VOSRDY)
            uint32_t voltage_scaling_output_ready  : 1;    // bit 14
            /// (reserved)
        ...
        //+=MEMORY======================================+
        /// Move Assignment is deleted
        ControlStatus& operator=(ControlStatus&&) = delete;
        /// Assignment from a volatile to a non volatile copy of the register.
        /// @note Does not return a reference
        inline void operator=(ControlStatus volatile& other) { whole = other.whole; }
        /// Assignment from a non volatile to a volatile register
        /// @note Does not return a reference
        inline void operator=(ControlStatus const& other) volatile { whole = other.whole; }
        /// Copy Assign a complete value from the base type uint32_t into a volatile version
        /// This allows you to assign the whole register to a literal value or a known constant of the same type.
        /// @note Does not return a reference
        inline void operator=(uint32_t const& value) volatile { whole = value; }
    ...
    // Ensure the register is in standard layout format
    static_assert(std::is_standard_layout<ControlStatus>::value, "Must be standard layout");
    // Ensure the sizeof the entire register is correct.
    static_assert(sizeof(ControlStatus) == 4UL, "Must be this exact size");

    //+=MEMORY======================================+
    /// power control register (CR)
    Control control;    // offset 0x0UL
    /// power control/status register (CSR)
    ControlStatus control_status;    // offset 0x4UL
...
static_assert(offsetof(PowerController, control) == 0x0UL, "Must be located at this offset");
static_assert(offsetof(PowerController, control_status) == 0x4UL, "Must be located at this offset");
