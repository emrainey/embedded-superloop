...
usart::Driver* usart_instances[8] = {nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr};    // 4, 5, 7, 8
/// Array of USART statistics pointers for interrupt handling
usart::Statistics* usart_statistics[8] = {nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr};    // 4, 5, 7, 8
...
/// Interrupt service routine for USART4
void usart4_isr(void) {
    cortex::extended_vector_statistics
        .count[polyfill::to_underlying(stm32::InterruptRequest::UniversalSynchronousAsynchronousReceiverTransmitter4)]++;
    if (usart_instances[3]) {
        usart_instances[3]->HandleInterrupt();
    }
}

/// Interrupt service routine for USART5
void usart5_isr(void) {
    cortex::extended_vector_statistics
        .count[polyfill::to_underlying(stm32::InterruptRequest::UniversalSynchronousAsynchronousReceiverTransmitter5)]++;
    if (usart_instances[4]) {
        usart_instances[4]->HandleInterrupt();
    }
}
...
void usart6_isr(void) {
    ...
    if (usart_instances[5]) {
        usart_instances[5]->HandleInterrupt();
    }
}

/// Interrupt service routine for USART7
void usart7_isr(void) {
    cortex::extended_vector_statistics
        .count[polyfill::to_underlying(stm32::InterruptRequest::UniversalSynchronousAsynchronousReceiverTransmitter7)]++;
    if (usart_instances[6]) {
        usart_instances[6]->HandleInterrupt();
    }
}

/// Interrupt service routine for USART8
void usart8_isr(void) {
    cortex::extended_vector_statistics
        .count[polyfill::to_underlying(stm32::InterruptRequest::UniversalSynchronousAsynchronousReceiverTransmitter8)]++;
    if (usart_instances[7]) {
        usart_instances[7]->HandleInterrupt();
    }
}

namespace usart {
Driver::Driver(
    h7xx::UniversalSynchronousAsynchronousReceiverTransmitter volatile& usart, dma::Manager& dma_driver, cortex::Peripheral rx_peripheral,
...
void Driver::ComputeBaudRate(uint32_t baud_rate) const {
    ...
    stm32::h7xx::UniversalSynchronousAsynchronousReceiverTransmitter::BaudRate brr;
    stm32::h7xx::UniversalSynchronousAsynchronousReceiverTransmitter::Control1 control1;
    ...
    brr.bits.baud_rate_mantissa_bits_4_to_15 = mantissa & 0xFFF;
    brr.bits.baud_rate_fraction_bits_0_to_3 = fraction & 0x0F;
    ...
    if constexpr (debug::Usart) {
        jarnax::print(
            ...
            static_cast<uint32_t>(brr.bits.baud_rate_mantissa_bits_4_to_15),
            static_cast<uint32_t>(brr.bits.baud_rate_fraction_bits_0_to_3)
        );
    }
}

uint32_t Driver::GetBaudRate(void) const {
    stm32::h7xx::UniversalSynchronousAsynchronousReceiverTransmitter::Control1 control1 = usart_.control1;    // read
    stm32::h7xx::UniversalSynchronousAsynchronousReceiverTransmitter::BaudRate brr = usart_.baud_rate;        // read
    // uint32_t over8u = control1.bits.oversampling_mode == 1 ? 8 : 16;
    float over8f = control1.bits.oversampling_mode ? 8.0f : 16.0f;
    float divider =
        (static_cast<float>(brr.bits.baud_rate_mantissa_bits_4_to_15) + (static_cast<float>(brr.bits.baud_rate_fraction_bits_0_to_3) / over8f));
...
core::Status Driver::Initialize(core::units::Hertz peripheral_frequency) {
    ...
    stm32::h7xx::UniversalSynchronousAsynchronousReceiverTransmitter::Control1 control1;
    stm32::h7xx::UniversalSynchronousAsynchronousReceiverTransmitter::Control2 control2;
    stm32::h7xx::UniversalSynchronousAsynchronousReceiverTransmitter::Control3 control3;
    // disable the USART
    control1 = usart_.control1;    // read
...
core::Status Driver::Configure(uint32_t desired_baud_rate, bool parity, uint8_t stop_bits) {
    ...
    control1.bits.usart_enable = 1;                          // USART enabled
    // control2
    control2.whole = 0U;                             // clear
    ...
    std::uintptr_t source = reinterpret_cast<std::uintptr_t>(&usart_.receive_data.whole);
...
void Driver::HandleInterrupt(void) {
    h7xx::UniversalSynchronousAsynchronousReceiverTransmitter::InterruptStatus status = usart_.interrupt_status;    // read
    ...
    if (status.bits.transmit_buffer_empty) {
        if constexpr (configure::use_usart_as == configure::Mode::Interrupt) {
            if (tx_index_ < tx_span_.count()) {
                ...
                usart_.transmit_data.bits.transmit_data = tx_span_[tx_index_++];    // write
                // clear the TC flag too since we aren't done yet
                h7xx::UniversalSynchronousAsynchronousReceiverTransmitter::InterruptClear clear;
                clear.whole = 0U;
                clear.bits.transmission_complete_clear_flag = 1;    // clear TC flag
                usart_.interrupt_clear = clear;                     // write
                statistics_.bytes_transmitted++;
            } else {
                // disable TXE interrupt in Control1
                h7xx::UniversalSynchronousAsynchronousReceiverTransmitter::Control1 control1 = usart_.control1;    // read
    ...
    // Clear all the flags by writing 1 to the interrupt clear register
    h7xx::UniversalSynchronousAsynchronousReceiverTransmitter::InterruptClear clear;
    clear.whole = 0U;
    clear.bits.parity_error_clear_flag = 1;             // clear PE flag
    clear.bits.framing_error_clear_flag = 1;            // clear FE flag
    clear.bits.noise_clear_flag = 1;                    // clear NF flag
    clear.bits.overrun_error_clear_flag = 1;            // clear ORE flag
    clear.bits.idle_line_clear_flag = 1;                // clear IDLE flag
    clear.bits.transmission_complete_clear_flag = 1;    // clear TC flag
    usart_.interrupt_clear = clear;                     // write
}
...
core::Status Driver::Enqueue(core::Span<DataUnit const> const& data) {
    ...
    h7xx::UniversalSynchronousAsynchronousReceiverTransmitter::Control1 control1;
    if (tx_ready_) {
        ...
        if constexpr (configure::use_usart_as == configure::Mode::Dma) {
            ...
            tx_dma_resource_->ConfigureCopyToPeripheral(tx_span_, reinterpret_cast<std::uintptr_t>(&usart_.transmit_data.whole));
            // enable the DMA transmit stream in the USART
            h7xx::UniversalSynchronousAsynchronousReceiverTransmitter::Control3 control3 = usart_.control3;    // read
            control3.bits.direct_memory_access_transmitter = 1;                                                // DMA transmitter enabled
            usart_.control3 = control3;                                                                        // write
            // clear the TC flag to indicate that the DMA is ready to send more data
            h7xx::UniversalSynchronousAsynchronousReceiverTransmitter::InterruptClear clear_reg;
            clear_reg.whole = 0U;
            clear_reg.bits.transmission_complete_clear_flag = 1;    // clear TC flag
            usart_.interrupt_clear = clear_reg;                     // write
