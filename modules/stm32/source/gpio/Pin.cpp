...
Pin& Pin::SetMode(Mode mode) {
    auto pin_mode = general_purpose_input_output[polyfill::to_underlying(port_)].mode;               // read
    pin_mode.whole &= ~(0b11U << (index_ * 2U));                                                     // modify
    pin_mode.whole |= static_cast<std::uint32_t>(polyfill::to_underlying(mode) << (index_ * 2U));    // modify
    general_purpose_input_output[polyfill::to_underlying(port_)].mode = pin_mode;                    // write
    return *this;
}

Mode Pin::GetMode() const {
    auto mode = general_purpose_input_output[polyfill::to_underlying(port_)].mode;    // read
    return static_cast<Mode>((mode.whole >> (index_ * 2U)) & 0b11);                   // extract
}

Pin& Pin::SetOutputSpeed(Speed speed) {
    auto output_speed = general_purpose_input_output[polyfill::to_underlying(port_)].output_speed;        // read
    output_speed.whole &= ~(0b11U << (index_ * 2U));                                                      // modify
    output_speed.whole |= static_cast<std::uint32_t>(polyfill::to_underlying(speed) << (index_ * 2U));    // modify
    general_purpose_input_output[polyfill::to_underlying(port_)].output_speed = output_speed;             // write
    return *this;
}
...
Pin& Pin::SetOutputType(OutputType type) {
    auto output_type = general_purpose_input_output[polyfill::to_underlying(port_)].output_type;    // read
    output_type.whole &= ~static_cast<std::uint32_t>(1U << index_);                                 // modify
    output_type.whole |= static_cast<std::uint32_t>(polyfill::to_underlying(type) << index_);       // modify
    general_purpose_input_output[polyfill::to_underlying(port_)].output_type = output_type;         // write
    return *this;
}
...
Pin& Pin::SetResistor(Resistor type) {
    auto pull = general_purpose_input_output[polyfill::to_underlying(port_)].pullup_pulldown;    // read
    pull.whole &= ~(0b11U << (index_ * 2U));                                                     // modify
    pull.whole |= static_cast<std::uint32_t>(polyfill::to_underlying(type) << (index_ * 2U));    // modify
    general_purpose_input_output[polyfill::to_underlying(port_)].pullup_pulldown = pull;         // write
    return *this;
}
...
bool Pin::Value() const {
    Mode mode = GetMode();
    if (mode == Mode::Input) {
        auto input = general_purpose_input_output[polyfill::to_underlying(port_)].input_data;      // read
        return ((input.whole >> index_) & 0b1) > 0;                                                // extract
    } else if (mode == Mode::Output) {
        auto output = general_purpose_input_output[polyfill::to_underlying(port_)].output_data;    // read
...
void Pin::Value(bool value) {
    ...
    if (mode == Mode::Output) {
        ...
        general_purpose_input_output[polyfill::to_underlying(port_)].output_data = output;         // write
    } else {
        // nothing happens
    }
}
