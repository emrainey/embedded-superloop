...
public:
    ...
    SPIDriverTest()
        : timer_{}
        , mock_dma_manager_{}
        , spi_driver_{stm32::f4xx::spi1, mock_dma_manager_, SPI1_TX, SPI1_RX}
        , txn_{timer_}
    ...
    void SetUp() override {
        ...
        stm32::f4xx::SerialPeripheralInterface::Control1 control1;
        stm32::f4xx::SerialPeripheralInterface::Control2 control2;
        control1 = stm32::f4xx::spi1.control1;
        control2 = stm32::f4xx::spi1.control2;
    ...
    void SimulateDataTransfer(size_t count, uint8_t expected_tx[], uint8_t injected_rx[]) {
        if constexpr (stm32::configure::use_spi_as != stm32::configure::Mode::Dma) {
            for (size_t i = 0; i < count; ++i) {
                ...
                stm32::f4xx::SerialPeripheralInterface::Status status;
                stm32::f4xx::SerialPeripheralInterface::Data data;
                status = stm32::f4xx::spi1.status;
                status.bits.transmit_buffer_empty = 1;    // TXE
                stm32::f4xx::spi1.status = status;        // write
                ...
                data = stm32::f4xx::spi1.data;    // read the data register
                ASSERT_EQ(expected_tx[i], static_cast<uint8_t>(data.bits.data)) << "i=" << i;
                data.whole = injected_rx[i];
                stm32::f4xx::spi1.data = data;               // write the injected RX
