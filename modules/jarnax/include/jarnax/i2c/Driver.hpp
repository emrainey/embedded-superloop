...
public:
    ...
    /// The number of bytes to send. In a Write, this is the number of bytes after the address byte.
    std::size_t send_size;
    /// The number of bytes that have sent (not including the address byte).
    std::size_t sent_size;
    /// The offset to the location to start receiving data in the Buffer (only used in a Read transaction).
    std::size_t receive_offset;
    /// The number of bytes to receive (only used in a Read transaction).
